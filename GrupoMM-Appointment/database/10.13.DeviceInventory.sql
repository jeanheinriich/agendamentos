...
-- =====================================================================
-- O controle das operações de movimentação de dispositivos (Simcards e
-- equipamentos entre estoques e/ou técnicos/prestadores de serviços.
-- =====================================================================

-- ---------------------------------------------------------------------
-- Transações nos Simcards
-- ---------------------------------------------------------------------
-- Gatilho para lidar com as operações dos SIM Cards
-- ---------------------------------------------------------------------
CREATE OR REPLACE FUNCTION erp.simCardTransaction()
RETURNS trigger AS $$
DECLARE
  ...
  reason        varchar(100);
BEGIN
  -- Lida com a movimentação do SIM Card. Faz uso da variável especial
  ...
  IF (TG_OP = 'INSERT') THEN
    IF (TG_WHEN = 'BEFORE') THEN
      ...
      -- 3. Não está instalado em um equipamento
      NEW.equipmentID := null;
      -- 4. Não está indicado nenhum Slot do equipamento
      NEW.slotNumber := 0;
      ...
      IF (NEW.depositID IS NULL) THEN
        RAISE
          'O ID do depósito onde o SIM Card será armazenado não pode ser nulo'
          USING ERRCODE = 'not_null_violation';
      END IF;
    ELSIF (TG_WHEN = 'AFTER') THEN
      -- Registramos a aquisição do SIM Card
      INSERT INTO erp.deviceOperationLogs (contractorID, deviceType,
        ...
        (NEW.contractorID, 'SimCard', NEW.simcardID, 'Acquired', 
  ...
  ELSIF (TG_OP = 'UPDATE') THEN
    IF (TG_WHEN = 'BEFORE') THEN
      ...
      -- Verifica se estamos bloqueando o SIM Card
      IF (NEW.blocked = true) THEN
        IF (OLD.storageLocation <> 'StoredOnDeposit') THEN
          ...
          CASE (NEW.storageLocation)
            WHEN 'Installed' THEN
              reason := 'está instalado em um equipamento';
          ...
          RAISE
            'Você não pode bloquear um SIM Card que %', reason
            USING ERRCODE = 'restrict_violation';
        END IF;
      END IF;

      -- Verifica se estamos realizando uma movimentação do SIM Card
      IF ( (OLD.storageLocation <> NEW.storageLocation) OR
           ...
           (OLD.equipmentID <> NEW.equipmentID) OR
           (OLD.slotNumber <> NEW.slotNumber) ) THEN
        -- Verifica se o SIM Card encontra-se bloqueado
        IF (OLD.blocked = true) THEN
          IF (NEW.storageLocation <> 'StoredOnDeposit') THEN
            -- Não podemos movimentar um SIM Card bloqueado, então
            -- determina o motivo
            CASE (NEW.storageLocation)
              WHEN 'Installed' THEN
                reason := 'instalar';
            ...
            RAISE
              'Você não pode % um SIM Card que está bloqueado', reason
              USING ERRCODE = 'restrict_violation';
          END IF;
        END IF;
        ...
        CASE (NEW.storageLocation)
          WHEN 'StoredOnDeposit' THEN
            -- Quando especificado um depósito, verifica se foi
            -- informado o ID dele
            IF (NEW.depositID IS NULL) THEN
              RAISE
                'O ID do depósito onde o SIM Card será armazenado não pode ser nulo'
            ...
            -- sendo armazenando num depósito
            -- 1. Não está associado à um técnico
            NEW.technicianID := null;
            -- 2. Não está associado à um prestador de serviços
            NEW.serviceProviderID := null;
            ...
            NEW.slotNumber := 0;
          WHEN 'Installed' THEN
            -- Quando especificado que está instalado em um equipamento,
            -- verifica se foi informado o ID do equipamento e o número
            -- do Slot em que o mesmo foi instalado
            IF (NEW.equipmentID IS NULL) THEN
              RAISE
                'O ID do equipamento onde o SIM Card será instalado não pode ser nulo'
                USING ERRCODE = 'not_null_violation';
            END IF;
            IF (NOT NEW.slotNumber BETWEEN 1 AND 9) THEN
              RAISE
                'O número do slot do equipamento onde o SIM Card será instalado deve ser indicado'
                USING ERRCODE = 'not_null_violation';
            END IF;
            ...
            CASE (OLD.storageLocation)
              WHEN 'ReturnedToSupplier' THEN
                -- Estamos tentando instalar um SIM Card que não está
                -- mais de posse do contratante (está de posse do
                -- fornecedor)
                RAISE
                  'Você não pode instalar um SIM Card que está de posse do fornecedor'
                  USING ERRCODE = 'restrict_violation';
              WHEN 'UnderMaintenance' THEN
                -- Estamos tentando instalar um SIM Card que está em
                -- manutenção
                RAISE
                  'Você não pode instalar um SIM Card que está em manutenção'
                  USING ERRCODE = 'restrict_violation';
              ELSE
          ...
          WHEN 'StoredWithTechnician' THEN
            ...
            IF (NEW.technicianID IS NULL) THEN
              RAISE
                'O ID do técnico que está de posse do SIM Card não pode ser nulo'
                USING ERRCODE = 'not_null_violation';
            END IF;
            ...
            NEW.serviceProviderID := null;
            -- 2. Não está instalado em um equipamento
            NEW.equipmentID := null;
            -- 3. Não está indicado nenhum Slot do equipamento
            NEW.slotNumber := 0;

            -- Verifica de onde está saíndo
            IF (OLD.storageLocation = 'ReturnedToSupplier') THEN
              -- Estamos tentando enviar um SIM Card que não está mais
              -- de posse do contratante (está de posse do fornecedor)
              RAISE
                'Você não pode enviar um SIM Card que está de posse do fornecedor'
                USING ERRCODE = 'restrict_violation';
              END IF;
          WHEN 'StoredWithServiceProvider' THEN
            ...
            IF (NEW.serviceProviderID IS NULL) THEN
              RAISE
                'O ID do prestador de serviços que está de posse do SIM Card não pode ser nulo'
                USING ERRCODE = 'not_null_violation';
            END IF;
          ...
          WHEN 'UnderMaintenance' THEN
            ...
            IF (NEW.serviceProviderID IS NULL) THEN
              RAISE
                'O ID do prestador de serviços que fará a manutenção do SIM Card não pode ser nulo'
                USING ERRCODE = 'not_null_violation';
            END IF;
            ...
            IF (OLD.storageLocation = 'ReturnedToSupplier') THEN
              ...
              -- de posse do contratante (está de posse do fornecedor)
              RAISE
                'Você não pode enviar para manutenção um SIM Card que está de posse do fornecedor'
                USING ERRCODE = 'restrict_violation';
            END IF;
          WHEN 'ReturnedToSupplier' THEN
            -- Força as demais informações em função do mesmo estar sendo
            ...
            CASE (OLD.storageLocation)
              WHEN 'Installed' THEN
                -- Estamos tentando devolver um SIM Card que está
                -- instalado
                RAISE
                  'Você não pode devolver ao fornecedor um SIM Card que ainda está instalado'
                  USING ERRCODE = 'restrict_violation';
              WHEN 'StoredWithTechnician', 'StoredWithServiceProvider' THEN
                -- Estamos tentando devolver um SIM Card que está de
                -- posse de terceiros
                RAISE
                  'Você não pode devolver ao fornecedor um SIM Card que ainda está de posse de terceiros'
                  USING ERRCODE = 'restrict_violation';
              ELSE
                -- Prossegue normalmente
            END CASE;
      ...
      -- Força indicar que o sistema atualizou
    ...
    ELSIF (TG_WHEN = 'AFTER') THEN
      IF ( (OLD.storageLocation <> NEW.storageLocation) OR
          ...
          WHEN 'StoredWithTechnician' THEN
            ...
            CASE (OLD.storageLocation)
              ...
              WHEN 'StoredWithTechnician' THEN
                ...
                END IF;
              WHEN 'UnderMaintenance' THEN
                -- Estamos devolvendo um SIM Card que estava em
                -- manutenção
                logOperation := true;
                operation    := 'Returned';
              ELSE
        ...
        IF (logOperation = true) THEN
          -- Registramos a operação do SIM Card
          INSERT INTO erp.deviceOperationLogs (contractorID, deviceType,
            deviceID, operation, storageLocation, installedAt, slotNumber,
            technicianID, serviceProviderID, depositID, assignedToID,
            performedAt, performedByUserID) VALUES
            (OLD.contractorID, 'SimCard', OLD.simcardID, operation, 
             NEW.storageLocation, NEW.equipmentID, NEW.slotNumber,
             NEW.technicianID, NEW.serviceProviderID, NEW.depositID,
             NEW.assignedToID, NEW.updatedAt, NEW.updatedByUserID);
        END IF;
      END IF;
    END IF;
  ...
  ELSIF (TG_OP = 'DELETE') THEN
    ...
    DELETE FROM erp.deviceOperationLogs
     WHERE deviceType = 'SimCard'
       AND deviceID = OLD.simcardID;
...
CREATE TRIGGER simCardTransactionTriggerBefore
  BEFORE INSERT OR UPDATE OR DELETE ON erp.simCards
  FOR EACH ROW EXECUTE PROCEDURE erp.simCardTransaction();
CREATE TRIGGER simCardTransactionTriggerAfter
  AFTER INSERT OR UPDATE OR DELETE ON erp.simCards
  FOR EACH ROW EXECUTE PROCEDURE erp.simCardTransaction();
    ...
    ELSIF (TG_WHEN = 'AFTER') THEN
      IF ( (OLD.storageLocation = 'Installed') AND
        ...
        IF ( (OLD.installedAt <> NEW.installedAt) AND
          ...
          -- Atualizamos o registro que armazena a informação de quando
          -- o equipamento foi instalado neste veículo
          -- RAISE NOTICE 'Registramos a data de instalação e em qual instalação ocorreu';
          assignedToID := CASE
            WHEN NEW.assignedToID IS NULL THEN NEW.contractorID
            ELSE NEW.assignedToID
          END;
          UPDATE erp.installationRecords
             SET installedat = NEW.installedAt,
                 installationID = NEW.installationID
           WHERE contractorID = assignedToID
        ...
        CASE (NEW.storageLocation)
          ...
          WHEN 'Installed' THEN
            ...
            INSERT INTO erp.installationRecords
                   ...
                   (assignedToID, NEW.equipmentID, NEW.vehicleID,
      ...
      IF (logOperation = true) THEN
        ...
        INSERT INTO erp.deviceOperationLogs (contractorID, deviceType,
          ...
          assignedToID, performedAt, performedByUserID) VALUES
          (OLD.contractorID, 'Equipment', OLD.equipmentID, operation,
           ...
           NEW.serviceProviderID, NEW.depositID, NEW.assignedToID,
           NEW.updatedAt, NEW.updatedByUserID);
      END IF;
    END IF;
