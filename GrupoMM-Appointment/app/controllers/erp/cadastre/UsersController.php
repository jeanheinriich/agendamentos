  ...
  extends Controller
{
  ...
  ): Response
  {
    ...
    try
    {
      ...
      // Limitamos sempre a informação de contratante e dos usuários ao
      // nível igual ou inferior ao do usuário que está gerenciando
      $contractorID = $this->authorization->getContractor()->id;
      $userLevel    = $this->authorization->getUser()->groupid;
      ...
      $sql = "SELECT U.level,
                     ...
                     U.forcenewpassword,
                     U.fullcount
                FROM erp.UsersWallets({$contractorID}, {$entityID},
                  {$userLevel}, 0, '{$searchValue}', '{$searchField}',
                  '{$ORDER}', {$start}, {$length}) AS U;"
      ;
  ...
  ): Response
  {
    ...
    if ($request->isPost()) {
      ...
      if ($this->validator->isValid()) {
        ...
        if ($proceed) {
          ...
          try
          {
            ...
            if (User::where("contractorid", $contractorID)
              ...
              // Define sempre o contratante, pois não podemos adicionar
              // administradores
              $userData['contractorid'] = $contractorID;
              ...
              if ($autogeneratepassword === "true") {
                ...
                if ($this->sendEmail('sendpassword', $mailData)) {
                  ...
                  $this->warning("Enviado e-mail com dados de acesso "
                    . "da conta para o usuário {username} do contratante "
                    . "{contractor}",
                    [ 'username' => $userData['username'],
                      'contractor' => $contractor->name ]
                  );
              ...
              // Registra o sucesso
              $this->info("Cadastrado o usuário '{username}' com "
                . "sucesso.",
                [ 'username'  => $userData['username'] ]
              );
              ...
              $this->debug("Redirecionando para {routeName}",
                [ 'routeName' => 'ERP\Cadastre\Users' ]
              );

              // Redireciona para a página de gerenciamento de usuários
              return $this->redirect($response, 'ERP\Cadastre\Users');
            } else {
              // Registra o erro
        ...
        $this->debug('Os dados do usuário são INVÁLIDOS');
    ...
    $this->breadcrumb->push('Adicionar',
      $this->path('ERP\Cadastre\Users\Add')
    );
  ...
  ): Response
  {
    ...
    try
    {
      ...
      $userID = $args['userID'];
      $user = User::leftjoin('entities', 'users.entityid',
            '=', 'entities.entityid'
          )
        ->where('users.contractorid', $contractorID)
        ...
        if ($this->validator->isValid()) {
          ...
          if ($proceed) {
            ...
            try
            {
              ...
              // Nunca permite a mudança de entidade nem de contratante
              unset($userData['entityid']);
    ...
    $this->breadcrumb->push('Editar',
      $this->path('ERP\Cadastre\Users\Edit', ['userID' => $userID])
    );
