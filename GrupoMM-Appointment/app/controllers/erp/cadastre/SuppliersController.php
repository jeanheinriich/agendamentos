 ...
 * Descrição:
 *
 * O controlador do gerenciamento dos fornecedores do sistema. Um
 * fornecedor pode ser uma pessoa física e/ou jurídica.
 */
...
use App\Models\Entity as Supplier;
  ...
  extends Controller
{
  ...
  ): array
  {
    ...
    if ($addition) {
      ...
      $validationRules['blocked'] = V::boolVal()
        ->setName('Bloquear este fornecedor e todas suas '
            . 'unidades/filiais'
          )
      ;
    }
  ...
  /**
   * Recupera as informações de números de telefones de um fornecedor.
   *
   * @param int $supplierID
   *   A ID do fornecedor para o qual desejamos obter esta informação
   * @param int $subsidiaryID
   *   A ID da unidade/filial deste fornecedor para o qual desejamos
  ...
  ): Collection
  {
    ...
    return Mailing::where('entityid', $supplierID)
      ->where('subsidiaryid', $subsidiaryID)
      ->get([
          'entityid',
          'subsidiaryid',
  ...
  /**
   * Recupera as informações de contatos adicionais de um fornecedor.
   *
   * @param int $contractorID
   *   A ID do contratante deste fornecedor
   * @param int $supplierID
   *   A ID do fornecedor para o qual desejamos obter esta informação
   * @param int $subsidiaryID
   *   A ID da unidade/filial deste fornecedor para o qual desejamos
  ...
  /**
   * Exibe a página inicial do gerenciamento de fornecedores.
   * 
  ...
  ): Response
  {
    ...
    $this->breadcrumb->push('Fornecedores',
      $this->path('ERP\Cadastre\Suppliers')
    );

    // Registra o acesso
    $this->info("Acesso ao gerenciamento de fornecedores.");

    // Recupera os dados da sessão
    $supplier = $this->session->get('supplier',
      [ 'searchField' => 'name',
        'searchValue' => '',
        'displayStart' => 0 ]
    );
    ...
    return $this->render($request, $response,
      'erp/cadastre/suppliers/suppliers.twig',
      [ 'supplier' => $supplier ])
    ;
  }

  /**
   * Recupera a relação dos fornecedores em formato JSON.
   * 
   * @param Request $request
   *   A requisição HTTP
   * @param Response $response
  ...
  {
    $this->debug("Acesso à relação de fornecedores.");
    ...
    try
    {
      ...
      $sql = "SELECT E.entityID as id,
                ...
                FROM erp.getEntitiesData({$contractor->id}, 0,
                  'supplier', '{$searchValue}', '{$searchField}',
                  NULL, NULL, NULL, {$start}, {$length}) as E;"
      ;
      $suppliers = $this->DB->select($sql);

      if (count($suppliers) > 0) {
        $rowCount = $suppliers[0]->fullcount;
        ...
        if (empty($searchValue)) {
          $error = "Não temos fornecedores cadastrados.";
        } else {
          switch ($searchField) {
            case 'subsidiaryname':
          ...
          $error = "Não temos fornecedores cadastrados cujo "
    ...
    catch(QueryException $exception)
    {
      ...
      $error = "Não foi possível recuperar as informações de "
        . "fornecedores. Erro interno no banco de dados."
      ;
    }
    catch(Exception $exception)
    {
  ...
  /**
   * Exibe um formulário para adição de um fornecedor, quando
  ...
  ): Response
  {
    ...
    catch (RuntimeException $exception)
    {
      ...
      $this->debug("Redirecionando para {routeName}",
        [ 'routeName' => 'ERP\Cadastre\Suppliers' ]
      );

      // Redireciona para a página de gerenciamento de fornecedores
      return $this->redirect($response, 'ERP\Cadastre\Suppliers');
    }
    ...
    if ($request->isPost()) {
      ...
      // Registra o acesso
      $this->debug("Processando à adição de fornecedor.");
      ...
      if ($this->validator->isValid()) {
        $this->debug('Os dados do fornecedor são VÁLIDOS');

        // Recupera os dados do fornecedor
        $supplierData = $this->validator->getValues();
        ...
        $allHasValid = true;
        foreach($supplierData['subsidiaries']
        ...
        if ($allHasValid) {
          try
          {
            // Primeiro, verifica se não temos um fornecedor com o mesmo
            // nome (razão social no caso de pessoa jurídica)
            if (Supplier::where("contractorid", '=', $contractor->id)
                  ->where("supplier", "true")
                  ->whereRaw("public.unaccented(name) ILIKE "
                      . "public.unaccented('{$supplierData['name']}')"
                    )
                  ->count() === 0) {
              // Agora verifica se não temos outra unidade/filial com o
              // mesmo cpf e/ou cnpj dos informados neste fornecedor
              $save = true;
              foreach($supplierData['subsidiaries'] as $subsidiary)
              {
                if (Supplier::join("subsidiaries", "entities.entityid",
                          '=', "subsidiaries.entityid"
                        )
                      ->where("entities.supplier", "true")
                  ...
                  // de fornecedor com o mesmo CPF/CNPJ
                  ...
                  $this->debug("Não foi possível inserir as "
                    ...
                    . "fornecedor '{name}'. Já existe outr{person} com "
              ...
              if ($save) {
                // Grava o novo fornecedor

                // Separamos as informações das unidades/filiais do
                // restante dos dados do fornecedor
                $subsidiariesData = $supplierData['subsidiaries'];
                unset($supplierData['subsidiaries']);

                // Iniciamos a transação
                $this->DB->beginTransaction();

                // Incluímos um novo fornecedor
                $supplier = new Supplier();
                $supplier->fill($supplierData);
                // Indicamos que é um fornecedor
                $supplier->supplier = true;
                // Adicionamos as demais informações
                $supplier->contractorid = $contractor->id;
                ...
                // Incluímos todas unidades/filiais deste fornecedor
                foreach($subsidiariesData as $subsidiaryData) {
                  ...
                  $subsidiary->entityid = $supplierID;
                  ...
                  foreach($contactsData as $contactData)
                  {
                    ...
                    $mailingAddress->entityid        = $supplierID;
                ...
                // Registra o sucesso
                $this->info("Cadastrado o fornecedor '{name}' com "
                  . "sucesso.",
                  [ 'name'  => $supplierData['name'] ]
                );

                // Alerta o usuário
                $this->flash("success", "O fornecedor <i>'{name}'</i> "
                ...
                return $this->redirect($response,
                  'ERP\Cadastre\Suppliers')
                ;
              }
            } else {
              // Registra o erro
              $this->debug("Não foi possível inserir as informações do "
                . "fornecedor '{name}'. Já existe outro fornecedor com "
                . "o mesmo nome.",
                [ 'name'  => $supplierData['name'] ]
              );

              // Alerta o usuário
              $this->flashNow("error", "Já existe um fornecedor com o "
                . "nome <i>'{name}'</i>.",
                [ 'name' => $supplierData['name'] ]
              );
            }
          ...
          catch(QueryException $exception)
          {
            ...
            $this->error("Não foi possível inserir as informações do "
              . "fornecedor '{name}'. Erro interno no banco de dados: "
              . "{error}",
              [ 'name'  => $supplierData['name'],
                'error' => $exception->getMessage() ]
            );
            ...
            $this->flashNow("error", "Não foi possível inserir as "
              . "informações do fornecedor. Erro interno no banco de "
              . "dados."
            );
          }
          catch(Exception $exception)
          {
            ...
            // Registra o erro
            $this->error("Não foi possível inserir as informações do "
              . "fornecedor '{name}'. Erro interno: {error}",
              [ 'name'  => $supplierData['name'],
                'error' => $exception->getMessage() ]
            );
            ...
            // Alerta o usuário
            $this->flashNow("error", "Não foi possível inserir as "
              . "informações do fornecedor. Erro interno."
            );
          }
        }
      } else {
        $this->debug('Os dados do fornecedor são INVÁLIDOS');
    ...
    // Exibe um formulário para adição de um fornecedor
    ...
    $this->breadcrumb->push('Adicionar',
      $this->path('ERP\Cadastre\Suppliers\Add')
    );

    // Registra o acesso
    $this->info("Acesso à adição de fornecedor.");

    return $this->render($request, $response,
      'erp/cadastre/suppliers/supplier.twig',
      [
        'formMethod' => 'POST',
  ...
  /**
   * Exibe um formulário para edição de um fornecedor, quando
   * solicitado, e confirma os dados enviados.
   * 
   * @param Request $request
   *   A requisição HTTP
  ...
  ): Response
  {
    ...
    try
    {
      ...
      $supplier = Supplier::join("entitiestypes", "entities.entitytypeid",
        ...
        ->where("entities.supplier", "true")
        ->where("entities.entityid", $supplierID)
      ...
      if ( $supplier->isEmpty() ) {
        throw new ModelNotFoundException("Não temos nenhum fornecedor "
          . "com o código {$supplierID} cadastrado"
        );
      }
      $supplier = $supplier
        ->first()
      ...
      $supplier['subsidiaries'] = Subsidiary::join("cities",
      ...
      foreach ($supplier['subsidiaries'] as $row => $subsidiary) {
        // Telefones
        $phones = $this
          ->getPhones($supplierID,
              $subsidiary['subsidiaryid']
            )
        ;
        if ( $phones->isEmpty() ) {
          // Criamos os dados de telefone em branco
          $supplier['subsidiaries'][$row]['phones'] = [
            [
              'phoneid' => 0,
              'phonetypeid' => 1,
              'phonenumber' => ''
        ...
        // E-mails
        $emails = $this
          ->getEmails($supplierID,
              $subsidiary['subsidiaryid']
            )
        ;
        if ( $emails->isEmpty() ) {
          // Criamos os dados de e-mail em branco
          $supplier['subsidiaries'][$row]['emails'] = [
            [
              'mailingid' => 0,
              'email' => ''
            ]
        ...
        // Contatos adicionais
        $contacts = $this
          ->getContacts($contractor->id, $supplierID,
              $subsidiary['subsidiaryid']
            )
        ;
        if ( !$contacts->isEmpty() ) {
          $supplier['subsidiaries'][$row]['contacts'] =
            $contacts->toArray()
          ;
        }
      }
    ...
    {
      // Registra o erro
      $this->error("Não foi possível localizar o fornecedor código "
        . "{supplierID}.",
    ...
    if ($request->isPut()) {
      ...
      // Registra o acesso
      $this->debug("Processando à edição do fornecedor '{name}'.",
        [ 'name' => $supplier['name'] ]
      );
      ...
      if ($this->validator->isValid()) {
        ...
        // Recupera os dados modificados do fornecedor
        $supplierData = $this->validator->getValues();
        ...
        if ($allHasValid) {
          try
          {
            ...
            // de que o mesmo é fornecedor
            unset($supplierData['entitytypeid']);
            unset($supplierData['supplier']);
            ...
            $subsidiaries = Subsidiary::where("entityid", $supplierID)
              ->get([
                  'subsidiaryid'
                ])
              ->toArray()
            ...
            $supplierChanged = Supplier::findOrFail($supplierID);
            $supplierChanged->fill($supplierData);
            $supplierChanged->updatedbyuserid =
              $this->authorization->getUser()->userid
            ;
            $supplierChanged->save();
            ...
            // Registra o sucesso
            $this->info("O fornecedor '{name}' foi modificado com "
              . "sucesso.",
              [ 'name' => $supplierData['name'] ]
            );

            // Alerta o usuário
            $this->flash("success", "O fornecedor <i>'{name}'</i> foi "
      ...
      $this->validator->setValues($supplier);
    }

    // Exibe um formulário para edição de um fornecedor
    ...
    $this->breadcrumb->push('Editar',
      $this->path('ERP\Cadastre\Suppliers\Edit', [
        'supplierID' => $supplierID
      ])
    );

    // Registra o acesso
    $this->info("Acesso à edição do fornecedor '{name}'.",
      [ 'name' => $supplier['name'] ]
    );
  ...
  {
    // Registra o acesso
    $this->debug("Processando à remoção de fornecedor.");
    ...
    try
    {
      ...
      $supplier->deleteCascade($logoDirectory);

      // Efetiva a transação
      $this->DB->commit();

      // Registra o sucesso
      $this->info("O fornecedor '{name}' foi removido com sucesso.",
        [ 'name' => $supplier->name ]
      );
      ...
      return $response
        ...
        ->withJson([
            ...
            'message' => "Removido o fornecedor {$supplier->name}",
            'data' => "Delete"
          ])
      ;
    }
    catch(ModelNotFoundException $exception)
    {
      ...
      // Registra o erro
      $this->error("Não foi possível localizar o fornecedor código "
        . "{supplierID} para remoção.",
        [ 'supplierID' => $supplierID ]
      );

      $message = "Não foi possível localizar o fornecedor para "
        . "remoção."
      ;
    }
    catch(QueryException $exception)
    {
      ...
      $message = "Não foi possível remover o fornecedor. Erro interno "
        . "no banco de dados."
      ;
    }
    catch(Exception $exception)
  ...
  /**
   * Alterna o estado do bloqueio de um fornecedor e/ou de uma
  ...
  ): Response
  {
    ...
    try
    {
      ...
      if (is_null($subsidiaryID)) {
        ...
        $message = "O fornecedor '{$supplier->name}' foi {$action} "
          . "com sucesso."
        ;
      } else {
        ...
        $message = "A unidade/filial '{$subsidiary->name}' do "
          . "fornecedor '{$supplier->name}' foi {$action} com sucesso."
        ;
      }
    ...
    catch(ModelNotFoundException $exception)
    {
      if (is_null($subsidiaryID)) {
        // Registra o erro
        $this->error("Não foi possível localizar o fornecedor código "
          . "{supplierID} para alternar o estado do bloqueio.",
          [ 'supplierID' => $supplierID ])
        ;
        ...
        $this->error("Não foi possível localizar a unidade/filial "
          . "código {subsidiaryID} do fornecedor código {supplierID} "
        ...
        $message = "Não foi possível localizar a unidade/filial do "
          . "fornecedor para alternar o estado do bloqueio."
        ;
      }
    }
    catch(QueryException $exception)
    {
      if (is_null($subsidiaryID)) {
        ...
        $this->error("Não foi possível alternar o estado do bloqueio "
          . "do fornecedor '{name}'. Erro interno no banco de dados: "
          . "{error}.",
          [ 'name'  => $supplier->name,
            'error' => $exception->getMessage() ]
        );
        ...
        // Registra o erro
        $this->error("Não foi possível alternar o estado do bloqueio "
          . "da unidade/filial {subsidiaryName} do fornecedor "
        ...
        $message = "Não foi possível alternar o estado do bloqueio da "
          . "unidade/filial do fornecedor. Erro interno no banco de "
          . "dados."
        ;
      }
    }
    catch(Exception $exception)
    {
      if (is_null($subsidiaryID)) {
        // Registra o erro
        $this->error("Não foi possível alternar o estado do bloqueio "
          . "do fornecedor '{name}'. Erro interno: {error}.",
          [ 'name'  => $supplier->name,
            'error' => $exception->getMessage() ]
        );
  ...
  /**
   * Gera um PDF para impressão das informações de um fornecedor.
   * 
   * @param Request $request
   *   A requisição HTTP
   * @param Response $response
  ...
  ): Response
  {
    ...
    $supplier['subsidiaries'] = $subsidiaryQry
    ...
    $title = "Dados cadastrais de fornecedor";
    $PDFFileName = "Supplier_ID_{$supplierID}.pdf";
    $page = $this->renderPDF(
      'erp/cadastre/suppliers/PDFsupplier.twig',
      [
        'supplier' => $supplier
      ]
    );
    ...
    $mpdf->SetSubject('Controle de fornecedores');
