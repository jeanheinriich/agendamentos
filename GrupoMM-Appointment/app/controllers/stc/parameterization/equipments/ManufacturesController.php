 ...
 * Descrição:
 * 
 * O controlador do gerenciamento de fabricantes de equipamentos
...
namespace App\Controllers\STC\Parameterization\Equipments;
  ...
  extends Controller
{
  ...
  public function show(Request $request, Response $response)
  {
    ...
    $this->breadcrumb->push('Fabricantes',
      $this->path('STC\Parameterization\Equipments\Manufactures')
    );
    
    // Registra o acesso
    $this->info("Acesso ao gerenciamento de fabricantes de "
      . "equipamentos."
    );
    
    // Recupera os dados da sessão
    $manufacture = $this->session->get('manufacture',
      [ 'name' => '' ]
    );
    
    // Renderiza a página
    return $this->render($request, $response,
      'stc/parameterization/equipments/manufactures/equipmentmanufactures.twig',
      [ 'manufacture' => $manufacture ])
    ;
  }

  /**
   * Recupera a relação dos fabricantes de equipamentos em formato JSON.
   * 
   * @param Request $request
   *   A requisição HTTP
   * @param Response $response
  ...
  {
    $this->debug("Acesso à relação de fabricantes de equipamentos.");
    ...
    try
    {
      ...
      // Inicializa a query
      $ManufactureQry = Manufacture::where('contractorid', '=',
        $contractor->id)
      ;
      ...
      if (count($manufactures) > 0) {
        $rowCount = $manufactures[0]->fullcount;
        ...
        if (empty($name)) {
          $error = "Não temos fabricantes de equipamentos cadastrados.";
        } else {
          $error = "Não temos fabricantes de equipamentos cadastrados "
    ...
    catch(QueryException $exception)
    {
      ...
      $this->error("Não foi possível recuperar as informações de "
        ...
        [ 'module' => 'fabricantes de equipamentos',
          'error'  => $exception->getMessage() ]
      );

      $error = "Não foi possível recuperar as informações de " 
        . "fabricantes de equipamentos. Erro interno no banco de "
        . "dados."
      ;
    }
    catch(Exception $exception)
    {
      ...
      $error = "Não foi possível recuperar as informações de "
        . "fabricantes de equipamentos. Erro interno."
      ;
    }
  ...
  /**
   * Sincroniza a relação dos fabricantes de equipamentos com o site do
   * STC, fazendo as devidas modificações na base de dados.
   * 
   * @param Request $request
   *   A requisição HTTP
   * @param Response $response
  ...
  public function synchronize(Request $request, Response $response)
  {
    ...
    $this->info("Processando o sincronismo da relação de "
      . "fabricantes de equipamentos com o site do STC."
    );
    ...
    // Inicializamos o serviço de obtenção dos fabricantes de veículos
    $manufactureService = new ManufactureService($synchronizer,
    ...
    $output = new ServerSentEventHandler(function ()
      use ($serverEvent, $manufactureService)
    {
      try {
  ...
  /**
   * Recupera a relação dos fabricantes de equipamentos em formato JSON
   * no padrão dos campos de preenchimento automático.
   * 
   * @param Request $request
   *   A requisição HTTP
   * @param Response $response
  ...
  public function getAutocompletionData(Request $request,
    Response $response)
  {
    $this->debug("Relação de fabricantes de equipamentos para "
    ...
    $this->debug("Acesso aos dados de preenchimento automático dos "
      . "fabricantes de equipamentos que contenham '{name}'",
      [ 'name' => $name ]
    );
    
    try
    {
      // Localiza os fabricantes de equipamentos na base de dados
      $message = "Fabricantes de equipamentos cujo nome contém "
        . "'{$name}'"
      ;
      $manufactures = Manufacture::where("contractorid", '=',
            $contractor->id
          )
        ->whereRaw("public.unaccented(name) ILIKE "
    ...
    catch(QueryException $exception)
    {
      ...
      $error = "Não foi possível recuperar as informações de "
        . "fabricantes de equipamentos para preenchimento automático. "
    ...
    return $response
      ...
      ->withJson([
          'result' => 'NOK',
          'params' => $request->getQueryParams(),
          'message' => "Não foi possível localizar fabricantes de "
