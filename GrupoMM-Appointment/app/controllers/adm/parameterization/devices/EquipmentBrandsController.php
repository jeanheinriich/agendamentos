 ...
 * Descrição:
 * 
 * O controlador do gerenciamento de marcas de equipamentos de
 * rastreamento.
 */
  ...
  extends Controller
{
  ...
  public function show(Request $request, Response $response)
  {
    ...
    $this->breadcrumb->push('Marcas',
      $this->path('ADM\Parameterization\Devices\Brands')
    );
    ...
    return $this->render($request, $response,
      'adm/parameterization/devices/brands/equipmentbrands.twig',
  ...
  public function get(Request $request, Response $response)
  {
    ...
    try
    {
      ...
      // Inicializa a query
      $EquipmentBrandQry = EquipmentBrand::query();
  ...
  public function add(Request $request, Response $response)
  {
    ...
    if ($request->isPost()) {
      ...
      if ($this->validator->isValid()) {
        ...
        try
        {
          ...
          // Primeiro, verifica se não temos uma marca de equipamento
          // com o mesmo nome
          if (EquipmentBrand::whereRaw("public.unaccented(name) ILIKE public.unaccented('{$equipmentBrandData['name']}')")
                ->count() === 0) {
            ...
            $this->debug("Redirecionando para {routeName}",
              [ 'routeName' => 'ADM\Parameterization\Devices\Brands' ]
            );
            
            // Redireciona para a página de gerenciamento de marcas de
            // equipamentos
            return $this->redirect($response,
              'ADM\Parameterization\Devices\Brands')
            ;
          } else {
            // Registra o erro
            $this->debug("Não foi possível inserir as informações da "
              . "marca de equipamento '{name}'. Já existe uma marca de "
        ...
        catch(QueryException $exception)
        {
          ...
          $this->error("Não foi possível inserir as informações da "
            . "marca de equipamento '{name}'. Erro interno no banco de "
            . "dados: {error}.",
            [ 'name'  => $equipmentBrandData['name'],
              'error' => $exception->getMessage() ]
          );
        ...
        catch(Exception $exception)
        {
          // Registra o erro
          $this->error("Não foi possível inserir as informações da "
            . "marca de equipamento '{name}'. Erro interno: {error}.",
            [ 'name'  => $equipmentBrandData['name'],
              'error' => $exception->getMessage() ]
          );
    ...
    $this->breadcrumb->push('Adicionar',
      $this->path('ADM\Parameterization\Devices\Brands\Add')
    );
    
    // Registra o acesso
    $this->info("Acesso à adição de marca de equipamento.");
    
    // Renderiza a página
    return $this->render($request, $response,
      'adm/parameterization/devices/brands/equipmentbrand.twig',
      [ 'formMethod' => 'POST' ])
    ;
  }
    ...
    array $args)
  {
    try
    {
      ...
      $equipmentBrand = EquipmentBrand::where('equipmentbrandid', '=', $equipmentBrandID)
        ->firstOrFail()
      ;
        ...
        if ($this->validator->isValid()) {
          ...
          try
          {
            ...
            if (strtolower($equipmentBrand['name']) != strtolower($equipmentBrandData['name'])) {
              // Modificamos o nome da marca de equipamento, então
              // verifica se temos uma marca de equipamento com o mesmo
              // nome neste contratante antes de prosseguir
              if (EquipmentBrand::whereRaw("public.unaccented(name) ILIKE "
                ...
                $this->debug("Não foi possível modificar as "
                  . "informações da marca de equipamento '{name}'. Já "
                  . "existe uma marca de equipamento com o mesmo nome.",
                  [ 'name'  => $equipmentBrandData['name'] ]
                );
          ...
          catch(QueryException $exception)
          {
            ...
            $this->error("Não foi possível modificar as informações da "
              . "marca de equipamento '{name}'. Erro interno no banco "
              . "de dados: {error}",
              [ 'name'  => $equipmentBrandData['name'],
                'error' => $exception->getMessage() ]
            );
    ...
    $this->breadcrumb->push('Editar',
      $this->path('ADM\Parameterization\Devices\Brands\Edit', [
        'equipmentBrandID' => $equipmentBrandID
      ])
    );
    ...
    array $args)
  {
    ...
    try
    {
      ...
      // Registra o sucesso
      $this->info("A marca de equipamento '{name}' foi removida com "
        . "sucesso.",
        [ 'name' => $equipmentBrand->name ]
      );
    ...
    catch(Exception $exception)
    {
      ...
      // Registra o erro
      $this->error("Não foi possível remover as informações da marca "
        . "de equipamento id '{id}'. Erro interno: {error}.",
        [ 'id'  => $equipmentBrandID,
          'error' => $exception->getMessage() ]
      );
