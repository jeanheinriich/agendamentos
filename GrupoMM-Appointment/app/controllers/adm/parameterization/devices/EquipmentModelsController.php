<?php
/*
 * Este arquivo é parte do Sistema de ADM do Grupo M&M
 *
 * (c) Grupo M&M
 *
...
use App\Models\OperatingFrequence;
  ...
  extends Controller
{
  ...
  ): array
  {
    $validationRules = [
      ...
      'operatingfrequenceid' => V::intVal()
        ->setName('ID da frequência de operação'),
  ...
  /**
   * Recupera as informações de frequências de operação.
   *
   * @return Collection
   *   A matriz com as informações de frequências de operação.
   *
   * @throws RuntimeException
   *   Em caso de não termos frequências de operação.
   */
  protected function getOperatingFrequencies(
    bool $toAdd = true
  ): Collection
  {
    try {
      // Recupera as informações de frequências de operação
      $operatingFrequencies = OperatingFrequence::orderBy('operatingfrequenceid')
        ->get([
            'operatingfrequenceid AS id',
            'name'
          ])
      ;

      if ( $operatingFrequencies->isEmpty() ) {
        throw new Exception("Não temos nenhuma frequência de operação cadastrada");
      }
    }
    catch (Exception $exception)
    {
      ...
      $this->error("Não foi possível obter as informações de "
        . "frequências de operação. Erro: {error}.",
        [ 'error' => $exception->getMessage() ]
      );

      throw new RuntimeException(
        "Não foi possível obter as frequências de operação"
      );
    }

    return $operatingFrequencies;
  }
  ...
  ): Response
  {
    ...
    $this->breadcrumb->push('Modelos',
      $this->path('ADM\Parameterization\Devices\Models')
    );
    ...
    try {
      ...
      // Recupera as informações de frequências de operação
      $operatingFrequencies = $this->getOperatingFrequencies();
    }
    catch (RuntimeException $exception)
    {
      ...
      $this->debug("Redirecionando para {routeName}",
        [ 'routeName' => 'ADM\Parameterization\Devices\Models' ]
      );

      // Redireciona para a página de gerenciamento de modelos de
      // equipamentos
      return $this->redirect($response,
        'ADM\Parameterization\Devices\Models'
      );
    }
    ...
    return $this->render($request, $response,
      'adm/parameterization/devices/models/equipmentmodels.twig',
      [
        'protocols' => $protocols,
        'operatingFrequencies' => $operatingFrequencies,
        'equipment' => $equipment,
      ])
    ;
  }
  ...
  ): Response
  {
    ...
    $operatingFrequenceID   = $postParams['operatingFrequenceID'];
    $operatingFrequenceName = $postParams['operatingFrequenceName'];
    ...
    $this->session->set('equipment',
      [ 'brand' => [
        ...
        'operatingFrequence' => [
          'id' => $operatingFrequenceID,
          'name' => $operatingFrequenceName
        ],
        'model' => [
          'name' => $name
        ]
    ...
    try
    {
      ...
      if (!empty($operatingFrequenceID)) {
        // Informado o ID da frequência de operação
        $EquipmentModelQry
          ->where('equipmentmodels.operatingfrequenceid', '=', $operatingFrequenceID)
        ;
      }
  ...
  ): Response
  {
      ...
      if ($this->validator->isValid()) {
        ...
        try
        {
          ...
          if (intval($equipmentModelData['equipmentbrandid']) > 0) {
            ...
            if (EquipmentModel::where("equipmentbrandid", '=',
          ...
          if ($save) {
            ...
            $this->info("Cadastrado o modelo de equipamento '{name}' "
              . "da marca '{brandName}' com sucesso.",
            ...
            $this->debug("Não foi possível inserir as informações do "
              . "modelo de equipamento '{name}' da marca "
              . "'{brandName}'. Já existe um modelo de equipamento com "
              . "o mesmo nome.",
              [ 'name'       => $equipmentModelData['name'],
                'brandName'  => $equipmentModelData['equipmentbrandname'] ]
            );
    ...
    $this->breadcrumb->push('Adicionar',
      $this->path('ADM\Parameterization\Devices\Models\Add')
    );
    
    // Registra o acesso
    $this->info("Acesso à adição de modelo de equipamento.");
    
    // Renderiza a página
    return $this->render($request, $response,
      'adm/parameterization/devices/models/equipmentmodel.twig',
      [
        'formMethod' => 'POST',
  ...
  ): Response
  {
          ...
          if ( ((strtolower($equipmentModel['name']) != strtolower($equipmentModelData['name'])) ||
            ...
            if (EquipmentModel::where("equipmentbrandid", '=',
              ...
              $this->debug("Não foi possível modificar as informações "
                ...
                . "'{brandName}'. Já existe um modelo de equipamento "
                . "com o mesmo nome.",
                [ 'name'       => $equipmentModelData['name'],
                  'brandName'  => $equipmentModelData['equipmentbrandname'] ]
              );
    ...
    $this->breadcrumb->push('Editar',
      $this->path('ADM\Parameterization\Devices\Models\Edit', ['equipmentModelID' => $equipmentModelID])
    );
  ...
  ): Response
  {
    ...
    {
      // Recupera as informações do modelo de equipamento
      $equipmentModel = EquipmentModel::where('equipmentmodelid', '=', $equipmentModelID)
        ->firstOrFail()
      ;
      ...
      // Registra o sucesso
      $this->info("O modelo de equipamento '{name}' foi removido com "
        . "sucesso.",
        [ 'name' => $equipmentModel->name ]
      );
    ...
    catch(QueryException $exception)
    {
      ...
      $this->error("Não foi possível remover as informações do modelo "
        . "de equipamento ID '{id}'. Erro interno no banco de dados: "
        . "{error}.",
        [ 'id'  => $equipmentModelID,
          'error' => $exception->getMessage() ]
      );
    ...
    catch(Exception $exception)
    {
      // Registra o erro
      $this->error("Não foi possível remover as informações do modelo "
        . "de equipamento '{id}'. Erro interno: {error}.",
        [ 'id'  => $equipmentModelID,
          'error' => $exception->getMessage() ]
      );
