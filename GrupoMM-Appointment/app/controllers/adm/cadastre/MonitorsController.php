 ...
 * Descrição:
 *
 * O controlador do gerenciamento dos empresas de monitoramento do sistema. Um empresa de monitoramento
...
use App\Models\Entity as Monitor;
  ...
  extends Controller
{
  ...
  ): array
  {
    $validationRules = [
      'entityid' => V::notBlank()
        ->intVal()
        ->setName('ID da empresa de monitoramento'),
      'name' => V::notBlank()
        ->length(2, 100)
    ...
    if ($addition) {
      ...
      $validationRules['blocked'] = V::boolVal()
        ->setName('Bloquear este empresa de monitoramento e todas suas '
            . 'unidades/filiais'
          )
      ;
    }
   ...
   *
   * @param int $monitorID
   *   A ID da empresa de monitoramento para o qual desejamos obter esta informação
   * @param int $subsidiaryID
   *   A ID da unidade/filial deste empresa de monitoramento para o qual desejamos
  ...
  ): Collection
  {
    ...
    return Mailing::where('entityid', $monitorID)
      ->where('subsidiaryid', $subsidiaryID)
      ->get([
          'entityid',
          'subsidiaryid',
   ...
   *
   * @param int $contractorID
   *   A ID do contratante deste empresa de monitoramento
   * @param int $monitorID
   *   A ID da empresa de monitoramento para o qual desejamos obter esta informação
   * @param int $subsidiaryID
   *   A ID da unidade/filial deste empresa de monitoramento para o qual desejamos
  ...
  /**
   * Exibe a página inicial do gerenciamento de empresas de monitoramento.
   * 
  ...
  ): Response
  {
    ...
    $this->breadcrumb->push('Empresas de monitoramento',
      $this->path('ADM\Cadastre\Monitors')
    );

    // Registra o acesso
    $this->info(
      "Acesso ao gerenciamento de empresas de monitoramento."
    );

    // Recupera os dados da sessão
    $monitor = $this->session->get('monitor',
      [ 'searchField' => 'name',
        'searchValue' => '',
        'displayStart' => 0 ]
    );
    ...
    return $this->render($request, $response,
      'adm/cadastre/monitors/monitors.twig',
      [ 'monitor' => $monitor ])
    ;
  }

  /**
   * Recupera a relação dos empresas de monitoramento em formato JSON.
   * 
   * @param Request $request
   *   A requisição HTTP
   * @param Response $response
  ...
  {
    $this->debug("Acesso à relação de empresas de monitoramento.");
    ...
    try
    {
      ...
      $sql = "SELECT E.entityID as id,
                     ...
                     E.createdat,
                     E.fullcount
                FROM erp.getEntitiesData(0, 0,
                  'monitor', '{$searchValue}', '{$searchField}',
                  NULL, NULL, NULL, {$start}, {$length}) as E;"
      ;
      $monitors = $this->DB->select($sql);

      if (count($monitors) > 0) {
        $rowCount = $monitors[0]->fullcount;
        ...
        if (empty($searchValue)) {
          $error = "Não temos empresas de monitoramento cadastradas.";
        } else {
          switch ($searchField) {
            case 'subsidiaryname':
    ...
    catch(QueryException $exception)
    {
      ...
      $this->error("Não foi possível recuperar as informações de "
        ...
        [ 'module' => 'empresas de monitoramento',
          'error'  => $exception->getMessage() ]
      );

      $error = "Não foi possível recuperar as informações de "
        . "empresas de monitoramento. Erro interno no banco de dados."
      ;
    }
    catch(Exception $exception)
    {
      ...
      $error = "Não foi possível recuperar as informações de "
        . "empresas de monitoramento. Erro interno."
      ;
    }
  ...
  /**
   * Exibe um formulário para adição de uma empresa de monitoramento,
  ...
  ): Response
  {
    ...
    catch (RuntimeException $exception)
    {
      ...
      $this->debug("Redirecionando para {routeName}",
        [ 'routeName' => 'ADM\Cadastre\Monitors' ]
      );

      // Redireciona para a página de gerenciamento de empresas de monitoramento
      return $this->redirect($response, 'ADM\Cadastre\Monitors');
    }
    ...
    if ($request->isPost()) {
      ...
      // Registra o acesso
      $this->debug("Processando à adição de empresa de monitoramento.");
      ...
      if ($this->validator->isValid()) {
        $this->debug('Os dados da empresa de monitoramento são VÁLIDOS');

        // Recupera os dados da empresa de monitoramento
        $monitorData = $this->validator->getValues();
        ...
        $allHasValid = true;
        foreach($monitorData['subsidiaries']
        ...
        if ($allHasValid) {
          try
          {
            // Primeiro, verifica se não temos uma empresa de monitoramento com o mesmo
            // nome (razão social no caso de pessoa jurídica)
            if (Monitor::where("contractorid", '=', $contractor->id)
                  ->where("monitor", "true")
                  ->whereRaw("public.unaccented(name) ILIKE "
                      . "public.unaccented('{$monitorData['name']}')"
                    )
                  ->count() === 0) {
              // Agora verifica se não temos outra unidade/filial com o
              // mesmo cpf e/ou cnpj dos informados neste empresa de monitoramento
              $save = true;
              foreach($monitorData['subsidiaries'] as $subsidiary)
              {
                if (Monitor::join("subsidiaries", "entities.entityid",
                          '=', "subsidiaries.entityid"
                        )
                      ->where("entities.monitor", "true")
                  ...
                  // de empresa de monitoramento com o mesmo CPF/CNPJ
                  ...
                  $this->debug("Não foi possível inserir as "
                    ...
                    . "empresa de monitoramento '{name}'. Já existe outr{person} com "
              ...
              if ($save) {
                // Primeiramente lida com as logomarcas da empresa de monitoramento
                ...
                // Grava a nova empresa de monitoramento

                // Separamos as informações das unidades/filiais do
                // restante dos dados da empresa de monitoramento
                $subsidiariesData = $monitorData['subsidiaries'];
                unset($monitorData['subsidiaries']);

                // Iniciamos a transação
                $this->DB->beginTransaction();

                // Incluímos um nova empresa de monitoramento
                $monitor = new Monitor();
                $monitor->fill($monitorData);
                // Indicamos que é uma empresa de monitoramento
                $monitor->monitor = true;
                // Adicionamos as demais informações
                $monitor->contractorid = 0;
                $monitor->createdbyuserid =
                  $this->authorization->getUser()->userid
                ;
                $monitor->updatedbyuserid =
                  $this->authorization->getUser()->userid
                ;
                $monitor->save();
                $monitorID = $monitor->entityid;

                // Incluímos todas unidades/filiais deste empresa de monitoramento
                foreach($subsidiariesData as $subsidiaryData) {
                  ...
                  foreach($contactsData as $contactData)
                  {
                    ...
                    $mailingAddress->entityid        = $monitorID;
                ...
                $newMonitor = Monitor::where('entityid', $monitorID)
                  ->first()
                ;
                $UUID = $newMonitor->entityuuid;
                $currentUserID =
                  $this->authorization->getUser()->userid
                ;

                // Agora renomeia os arquivos
                $monitorData['logonormal'] = $this->renameFile(
                  $logoDirectory,
                  $monitorData['logonormal'],
                  'Logo',
                  $UUID,
                ...
                $monitorData['logoinverted'] = $this->renameFile(
                  $logoDirectory,
                  $monitorData['logoinverted'],
                  'Logo',
                  $UUID,
                ...
                // Registra o sucesso
                $this->info("Cadastrado a empresa de monitoramento "
                  . "'{name}' com sucesso.",
                  [ 'name'  => $monitorData['name'] ]
                );

                // Alerta o usuário
                $this->flash("success", "A empresa de monitoramento "
                ...
                return $this->redirect($response,
                  'ADM\Cadastre\Monitors')
                ;
              }
            } else {
              // Registra o erro
              $this->debug("Não foi possível inserir as informações da "
                . "empresa de monitoramento '{name}'. Já existe outra "
                . "empresa de monitoramento com o mesmo nome.",
                [ 'name'  => $monitorData['name'] ]
              );
              ...
              $this->flashNow("error", "Já existe uma empresa de "
                . "monitoramento com o nome <i>'{name}'</i>.",
                [ 'name' => $monitorData['name'] ]
              );
            }
          }
          catch (UploadFileException $exception)
          {
            ...
            $this->error("Não foi possível inserir as informações da "
              . "empresa de monitoramento '{name}'. {error}",
              [ 'name'  => $monitorData['name'],
                'error' => $exception->getMessage() ]
            );
            ...
            $this->flashNow("error", "Não foi possível inserir as "
              . "informações da empresa de monitoramento. Erro interno "
              . "no banco de dados."
            );
          }
          catch(QueryException $exception)
          {
            ...
            // Registra o erro
            $this->error("Não foi possível inserir as informações da "
              . "empresa de monitoramento '{name}'. Erro interno no "
          ...
          catch(Exception $exception)
          {
            ...
            // Registra o erro
            $this->error("Não foi possível inserir as informações da "
              . "empresa de monitoramento '{name}'. Erro interno: "
              . "{error}",
              [ 'name'  => $monitorData['name'],
                'error' => $exception->getMessage() ]
            );
        ...
        $this->debug(
          'Os dados da empresa de monitoramento são INVÁLIDOS'
        );
    ...
    $this->breadcrumb->push('Adicionar',
      $this->path('ADM\Cadastre\Monitors\Add')
    );

    // Registra o acesso
    $this->info("Acesso à adição de empresa de monitoramento.");

    return $this->render($request, $response,
      'adm/cadastre/monitors/monitor.twig',
      [ 'formMethod' => 'POST',
  ...
  /**
   * Exibe um formulário para edição de uma empresa de monitoramento,
   * quando solicitado, e confirma os dados enviados.
   * 
   * @param Request $request
   *   A requisição HTTP
  ...
  ): Response
  {
    ...
    catch (RuntimeException $exception)
    {
      ...
      // Redireciona para a página de gerenciamento de empresas de
      // monitoramento
      return $this->redirect($response, 'ADM\Cadastre\Monitors');
    }
    ...
    try
    {
      // Recupera as informações da empresa de monitoramento
      $monitorID = $args['monitorID'];
      $monitor = Monitor::join("entitiestypes", "entities.entitytypeid",
        ...
        ->where("entities.monitor", "true")
        ->where("entities.entityid", $monitorID)
        ->where("entities.contractorid", '=', 0)
        ->get([
      ...
      if ( $monitor->isEmpty() ) {
        throw new ModelNotFoundException("Não temos nenhuma empresa de "
          . "monitoramento com o código {$monitorID} cadastrada"
        );
      }
      $monitor = $monitor
        ->first()
      ...
      if (count($files) > 0) {
        ...
        foreach ($files as $imageFile) {
          ...
          switch ($this->getImageSuffix($imageFile)) {
            case 'N':
              $monitor['logonormal'] = $imageDataBase64;

              break;
            case 'I':
              $monitor['logoinverted'] = $imageDataBase64;
      ...
      $monitor['subsidiaries'] = Subsidiary::join("cities",
      ...
      foreach ($monitor['subsidiaries'] as $row => $subsidiary) {
        // Telefones
        $phones = $this
          ->getPhones($monitorID,
        ...
        if ( $phones->isEmpty() ) {
          ...
          $monitor['subsidiaries'][$row]['phones'] = [
            [
              'phoneid' => 0,
              'phonetypeid' => 1,
              'phonenumber' => ''
        ...
        if ( $emails->isEmpty() ) {
          ...
          $monitor['subsidiaries'][$row]['emails'] = [
            [
              'mailingid' => 0,
              'email' => ''
            ]
        ...
        // Contatos adicionais
        $contacts = $this
          ->getContacts($contractor->id, $monitorID,
              $subsidiary['subsidiaryid']
            )
        ;
        if ( !$contacts->isEmpty() ) {
          $monitor['subsidiaries'][$row]['contacts'] =
            $contacts->toArray()
          ;
        }
      }
    ...
    {
      // Registra o erro
      $this->error("Não foi possível localizar a empresa de monitoramento código "
        . "{monitorID}.",
        [ 'monitorID' => $monitorID ]
      );
    ...
    if ($request->isPut()) {
      ...
      // Registra o acesso
      $this->debug("Processando à edição da empresa de monitoramento '{name}'.",
        [ 'name' => $monitor['name'] ]
      );
      ...
      if ($this->validator->isValid()) {
        ...
        // Recupera os dados modificados da empresa de monitoramento
        $monitorData = $this->validator->getValues();
        ...
        if ($allHasValid) {
          try
          {
            // Recupera a ID da empresa de monitoramento
            $UUID = $monitor['entityuuid'];
            ...
            // de que o mesmo é empresa de monitoramento
            unset($monitorData['entitytypeid']);
            unset($monitorData['monitor']);
            ...
            $subsidiaries = Subsidiary::where("entityid", $monitorID)
              ->get([
                  'subsidiaryid'
                ])
              ->toArray()
            ...
            // Grava as informações da empresa de monitoramento
            $monitorChanged = Monitor::findOrFail($monitorID);
            $monitorChanged->fill($monitorData);
            $monitorChanged->updatedbyuserid =
              $this->authorization->getUser()->userid
            ;
            $monitorChanged->save();
            ...
            // Registra o sucesso
            $this->info("A empresa de monitoramento '{name}' foi "
          ...
          catch(UploadFileException $exception)
          {
            ...
            $this->flashNow("error", "Não foi possível modificar as "
              . "informações da empresa de monitoramento. {error}",
              ['error' => $exception->getMessage()]
            );
          }
          catch(Exception $exception)
        ...
        $this->debug('Os dados da empresa de monitoramento são INVÁLIDOS');
        $messages = $this->validator->getFormatedErrors();
        foreach ($messages AS $message) {
          $this->debug($message);
        }
      ...
      $this->validator->setValues($monitor);
    }
    ...
    $this->breadcrumb->push('Editar',
      $this->path('ADM\Cadastre\Monitors\Edit', [
        'monitorID' => $monitorID
      ])
    );

    // Registra o acesso
    $this->info("Acesso à edição da empresa de monitoramento '{name}'.",
      [ 'name' => $monitor['name'] ]
    );
  ...
  /**
   * Remove a empresa de monitoramento.
   * 
   * @param Request $request
   *   A requisição HTTP
   * @param Response $response
  ...
  {
    // Registra o acesso
    $this->debug("Processando à remoção de empresa de monitoramento.");
    ...
    try
    {
      ...
      // Agora apaga a empresa de monitoramento
      ...
      $monitor->deleteCascade($logoDirectory);

      // Efetiva a transação
      $this->DB->commit();

      // Registra o sucesso
      $this->info("A empresa de monitoramento '{name}' foi removida "
        . "com sucesso.",
        [ 'name' => $monitor->name ]
      );
      ...
      return $response
        ...
        ->withJson([
            ...
            'message' => "Removido a empresa de monitoramento {$monitor->name}",
            'data' => "Delete"
          ])
      ;
    }
    catch(ModelNotFoundException $exception)
    {
      ...
      $this->error("Não foi possível localizar a empresa de "
        . "monitoramento código {monitorID} para remoção.",
        [ 'monitorID' => $monitorID ]
      );

      $message = "Não foi possível localizar a empresa de "
        . "monitoramento para remoção."
      ;
    }
    catch(QueryException $exception)
    {
      ...
      $this->error("Não foi possível remover as informações do "
        . "empresa de monitoramento '{name}'. Erro interno no banco de "
        . "dados: {error}",
        [ 'name'  => $monitor->name,
          'error' => $exception->getMessage() ]
      );

      $message = "Não foi possível remover a empresa de monitoramento. "
    ...
    catch(Exception $exception)
    {
      ...
      // Registra o erro
      $this->error("Não foi possível remover as informações do "
        . "empresa de monitoramento '{name}'. Erro interno: {error}",
        [ 'name'  => $monitor->name,
          'error' => $exception->getMessage() ]
      );

      $message = "Não foi possível remover a empresa de monitoramento. Erro interno.";
    }
  ...
  /**
   * Alterna o estado do bloqueio de uma empresa de monitoramento e/ou
   * de uma unidade/filial deste empresa de monitoramento.
   * 
   * @param Request $request
   *   A requisição HTTP
   * @param Response $response
  ...
  ): Response
  {
    ...
    try
    {
      ...
      if (is_null($subsidiaryID)) {
        // Desbloqueia a empresa de monitoramento
        $monitor = Monitor::findOrFail($monitorID);
        $action   = $monitor->blocked
          ? "desbloqueado"
          : "bloqueado"
        ;
        $monitor->blocked = !$monitor->blocked;
        $monitor->updatedbyuserid = $this
          ->authorization
          ->getUser()
          ->userid
        ;
        ...
        $message = "A empresa de monitoramento '{$monitor->name}' foi "
          . "{$action} com sucesso."
        ;
      } else {
        ...
        $message = "A unidade/filial '{$subsidiary->name}' da "
          . "empresa de monitoramento '{$monitor->name}' foi {$action} "
          . "com sucesso."
        ;
      }
    ...
    catch(ModelNotFoundException $exception)
    {
      if (is_null($subsidiaryID)) {
        // Registra o erro
        $this->error("Não foi possível localizar a empresa de "
          . "monitoramento código {monitorID} para alternar o estado "
          . "do bloqueio.",
          [ 'monitorID' => $monitorID ])
        ;

        $message = "Não foi possível localizar a empresa de "
          . "monitoramento para alternar o estado do bloqueio."
        ;
      } else {
        // Registra o erro
        $this->error("Não foi possível localizar a unidade/filial "
          . "código {subsidiaryID} da empresa de monitoramento código "
          . "{monitorID} para alternar o estado do bloqueio.",
          [ 'monitorID' => $monitorID,
            'subsidiaryID' => $subsidiaryID ])
        ;

        $message = "Não foi possível localizar a unidade/filial da "
          . "empresa de monitoramento para alternar o estado do "
          . "bloqueio."
        ;
      }
    }
    catch(QueryException $exception)
    {
      if (is_null($subsidiaryID)) {
        ...
        $this->error("Não foi possível alternar o estado do bloqueio "
          . "da empresa de monitoramento '{name}'. Erro interno no "
          . "banco de dados: "
          . "{error}.",
          [ 'name'  => $monitor->name,
            'error' => $exception->getMessage() ]
        ...
        $message = "Não foi possível alternar o estado do bloqueio do "
          . "empresa de monitoramento. Erro interno no banco de dados."
        ;
      } else {
        // Registra o erro
        $this->error("Não foi possível alternar o estado do bloqueio "
          . "da unidade/filial {subsidiaryName} da empresa de "
          . "monitoramento '{name}'. Erro interno no banco de dados: "
          . "{error}.",
          [ 'subsidiaryName'  => $subsidiary->name,
            'name'  => $monitor->name,
            'error' => $exception->getMessage() ]
        ...
        $message = "Não foi possível alternar o estado do bloqueio da "
          . "unidade/filial da empresa de monitoramento. Erro interno "
          . "no banco de dados."
        ;
      }
    }
    catch(Exception $exception)
    {
      if (is_null($subsidiaryID)) {
        // Registra o erro
        $this->error("Não foi possível alternar o estado do bloqueio "
          . "da empresa de monitoramento '{name}'. Erro interno: "
          . "{error}.",
          [ 'name'  => $monitor->name,
            'error' => $exception->getMessage() ]
        );
        ...
        $message = "Não foi possível alternar o estado do bloqueio do "
          . "empresa de monitoramento. Erro interno."
        ;
      } else {
        // Registra o erro
        $this->error("Não foi possível alternar o estado do bloqueio "
          . "da unidade/filial {subsidiaryName} da empresa de "
          . "monitoramento '{name}'. Erro interno: {error}.",
          [ 'subsidiaryName'  => $subsidiary->name,
            'name'  => $monitor->name,
            'error' => $exception->getMessage() ]
        );
  ...
  ): Response
  {
    ...
    $this->debug("Processando à geração de PDF com as informações "
      . "cadastrais de uma empresa de monitoramento."
    );
    ...
    $monitor['subsidiaries'] = $subsidiaryQry
    ...
    $title = "Dados cadastrais de empresa de monitoramento";
    $PDFFileName = "Monitor_ID_{$monitorID}.pdf";
    $page = $this->renderPDF(
      'adm/cadastre/monitors/PDFmonitor.twig',
      [
        'monitor' => $monitor
      ]
    );
    ...
    $mpdf->SetSubject('Controle de empresas de monitoramento');
